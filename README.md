...
## Parameter Overriding with fins_edit.json

A `fins_edit.json` file gives you the ability to override parameters of `fins.json` without modifying the file. When an IP is built, the second step after loading `fins.json` is to load `fins_edit.json`, meaning that any "overrides" will affect all stages of code generation, IP packaging, and simulation. The `fins_edit.json` files are automatically generated by FINS when modifying sub-IP parameters with the `params` key of the `ip` top-level FINS key.

In addition to overriding the top-level `params` key, there are two other top-level FINS keys that may be overridden.
* `name`: When this key exists in `fins_edit.json`, the IP project is generated with this new name. (In the future this may be changed to only the packaged name instead of the project name)
* `part`: This top-level key may be overridden by `fins_edit.json`.
