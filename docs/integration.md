...
The diagram below shows a basic Application-level Nodeset. There are inter-node connections, exported ports, and exported AXI interfaces. Not shown here are the generated clock domains as well as the modules that connect to the Nodeset's external ports in the platform. The `nodes`, `connections`, `clocks` and `port_exports` fields are detailed in the [FINS Nodeset JSON Schema](#Nodeset-Json-Schema).
...
1. Determine which FINS Nodes will be included, how they will be connected, what clock domains should exist, and which ports should be exported from the Nodeset. Reference [Nodes](#Nodeset-Json-Schema:-Nodes), [Connections](#Nodeset-Json-Schema:-Connections) and [Clocks](#Nodeset-Json-Schema:-Clocks) sections for more information on insantiating nodes and making connections.
...
5. The `clocks` key defines the clock domains of the Nodeset and which ports they are tied to (along with associated resets). A "properties" clock domain is always created by default for any Nodeset with one or more properties interfaces. To create clock domains via JSON see the [Clocks JSON section](#Nodeset-Json-Schema:-Clocks).
6. The `[hdl_]port_exports` key defines which Node ports should be exported (become the external ports) from the Nodeset. If unset, any unconnected ports are exported from a Nodeset.
...
| clocks            | dict[] | APP  | YES      |               | Required for Application-level Nodesets. This field is an array of clock nets. Each entry connects a clock to a list of ports on Nodes in the Nodeset. See [Clocks JSON section](#Nodeset-Json-Schema:-Clocks). |
| port_exports      | dict[] | APP  | NO       | open ports    | An array of ports to export and become the external ports of this Nodeset. By default, unconnected ports are exported. Each dict element contains the "node_name" for the port and the "net" or actual port name. | |
| hdl_port_exports  | dict[] | APP  | NO       | open ports    | An array of HDL ports (standard logic [vectors]) to export and become the external ports of this Nodeset. By default, unconnected ports are exported. Each dict element contains the "node_name" for the port and the "net" or actual port name. |
...
| Key                 | Type              | Mode | Required | Default Value | Description |
| ------------------- | ----------------- | ---- | -------- | ------------- | ----------- |
| source              | dict              | APP  | YES      |               | The source port or signal for this connection. This dict contains the "node_name" for the port and the "net" or actual port name. |
| destinations        | dict[]            | APP  | YES      |               | The destination ports or signals of this connection. Each dict element contains the "node_name" for the port and the "net" or actual port/signal name. |

### Nodeset JSON Schema: Clocks
In a FINS Application-level Nodeset, clock domains must be created and tied to each port in the Nodeset. 
A clock domain named "properties" is always created for any Nodeset that has one or more properties interfaces. Each dictionary element of the `clocks` array has the following fields:

| Key                 | Type              | Mode | Required | Default Value | Description |
| ------------------- | ----------------- | ---- | -------- | ------------- | ----------- |
| clock               | dict              | APP  | YES      |               | The clock domain to create. '_aclk' will be appended to this before being used in HDL. An '_aresetn' will be created and paired with this domain. |
| nets                | dict[]            | APP  | YES      |               | The list of nets on this clock domain. Each dict element contains the "node_name" for the port and the "net" or actual port name to place on this clock domain. |
