...
for d in sim_source_data['data']:
    sim_expected_data.append(d * test_middle_pkg.params['TEST_PARAM_INTEGER'] * test_middle_pkg.params['TEST_PARAM_INTEGER'])
...
if sim_source_data == sim_sink_data:
    print('PASS: test_out00 simulation data is correct')
else:
    print('ERROR: test_out00 simulation data is incorrect')
    print('    * Expected: {}'.format(sim_source_data))
    print('    * Received: {}'.format(sim_sink_data))
    sys.exit(1)

###################################################################################################
# Path: test_in01 --> test_out01
###################################################################################################
# Open our simulation input
sim_source_data = {'last':[], 'data':[], 'metadata':[]}
...
if sim_source_data == sim_sink_data:
    print('PASS: test_out01 simulation data is correct')
else:
    print('ERROR: test_out01 simulation data is incorrect')
    print('    * Expected: {}'.format(sim_source_data))
    print('    * Received: {}'.format(sim_sink_data))
    sys.exit(1)
